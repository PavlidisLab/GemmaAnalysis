...
/**
 * Used to count up links and to generate the link(gene pair) background
 * distribution, which could be used to estimate the false positive rates under
 * different levels of confirmation. When shuffling, there are two steps to
 * finish this process. The first step is to prepare the working table. Then the
 * analysis is done using the working table.
 * 
 * @author xwan
...
public class LinkShufflerCLI extends ExpressionExperimentManipulatingCLI {
	/*
	 * How many shuffled runs to do. 2 or 3 is enough to get a quick idea of
	 * what the results will look like; 100 is better for final analysis.
	 */
	private int numIterations = 2;

	/*
	 * Print out the link details for shuffled data sets. This is only useful
	 * for debugging (big files).
	 */
	...
	protected Exception doWork(String[] args) {
		...
		if (doRealAnalysis) { // Currently this is really just for debugging
			...
			LinkStatistics realStats = lss.analyze(expressionExperiments,
					genes, taxon.getCommonName(), false, true);
			System.out.println(realStats.getTotalLinkCount()
					+ " gene links in total");
			confStats = realStats.getLinkConfirmationStats();

			try {
				Writer linksOut = new BufferedWriter(new FileWriter(new File(
						"link-data.txt")));
		...
		for (int i = 0; i < numIterations; i++) {
			...
			LinkStatistics sr = lss.analyze(expressionExperiments, genes, taxon
					.getCommonName(), true, true);
			...
			if (doShuffledOutput) {
				try {
					Writer linksOut = new BufferedWriter(new FileWriter(
	...
	protected void buildOptions() {
		...
		Option iterationNum = OptionBuilder.hasArg().withArgName(
				" Shuffling iterations ").withDescription(
				" The number of shuffling iterations (default = 2) ")
				.withLongOpt("iterationNum").create('i');
		addOption(iterationNum);

		Option outputShuffledLinks = OptionBuilder.withArgName(
				"Shuffled output").withDescription(
				"Print out link details for shuffled data sets").withLongOpt(
				"outputShuffledData").create('l');
		addOption(outputShuffledLinks);

		Option realAnalysis = OptionBuilder.withArgName(
				"Real analysis (unshuffled)").withDescription(
				"Perform a real link analysis and output it to link-data.txt")
				.withLongOpt("realAnalysis").create('r');
		addOption(realAnalysis);

		Option outputFile = OptionBuilder.hasArg().isRequired().withArgName(
				"Output File").withDescription(
				"Output shuffle statistics to this file")
