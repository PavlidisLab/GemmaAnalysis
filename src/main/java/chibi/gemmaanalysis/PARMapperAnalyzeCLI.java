...
public class PARMapperAnalyzeCLI extends AbstractSpringAwareCLI {
    ...
    protected void buildOptions() {
        ...
        Option pargeneexpFileOption = OptionBuilder.hasArg().withDescription( "PAR-gene-experiment map filename" )
                .withDescription( "file of Par/Gene/Exps to read" ).withLongOpt( "pargeneexpFile" ).create( 'p' );
        ...
        Option checkPARprobeOption = OptionBuilder.withDescription( "PAR probe level rankings" ).create(
                "checkPARprobe" );
        addOption( checkPARprobeOption );

        Option checkGeneCorankOption = OptionBuilder.withDescription( "PAR and Gene rankings" ).create(
                "checkGeneCorank" );
        addOption( checkGeneCorankOption );

        Option checkGeneRankOption = OptionBuilder.withDescription( "Gene rankings" ).create( "checkGeneRank" );
        addOption( checkGeneRankOption );

        Option checkExperimentTypesOption = OptionBuilder
                .withDescription( "Prints detailed information on experiments" ).create( "checkExperimentTypes" );
        addOption( checkExperimentTypesOption );

        Option checkTroubledExperimentsOption = OptionBuilder.withDescription(
                "Determines which experiments are troubled" ).create( "checkTroubledExperiments" );
        addOption( checkTroubledExperimentsOption );

        Option checkUniqueProbeMappingsOption = OptionBuilder.withDescription( "[Optional: used with PARprobe]" )
                .create( "checkUniqueProbeMappings" );
        addOption( checkUniqueProbeMappingsOption );

        Option checkAllCoexpOption = OptionBuilder.withDescription( "Co-expression links for all PARs" ).create(
                "checkAllCoexp" );
        addOption( checkAllCoexpOption );

        Option checkAllCoexpInDBOption = OptionBuilder.withDescription( "Co-expression links in DB for PAR/gene pairs" )
                .create( "checkAllCoexpInDB" );
        addOption( checkAllCoexpInDBOption );

        Option checkAllPairsCoexpOption = OptionBuilder.withDescription( "All co-expression values for PAR/gene pairs" )
    ...
    protected Exception doWork( String[] args ) {
        ...
        try {
            ...
            if ( checkPAR || checkGeneRank ) {
                pxx = new PrintStream( new FileOutputStream( outFileDir + "/" + stem + ".output.xx.txt" ) );
                pxe = new PrintStream( new FileOutputStream( outFileDir + "/" + stem + ".output.xe.txt" ) );
                pex = new PrintStream( new FileOutputStream( outFileDir + "/" + stem + ".output.ex.txt" ) );
                pee = new PrintStream( new FileOutputStream( outFileDir + "/" + stem + ".output.ee.txt" ) );
            ...
            if ( checkGeneCorank ) {
                ...
                pgp_xx = new PrintStream( new FileOutputStream( outFileDir + "/" + stem + ".output.geneVsPar.xx.txt" ) );
                pgp_xe = new PrintStream( new FileOutputStream( outFileDir + "/" + stem + ".output.geneVsPar.xe.txt" ) );
         ...
         * batchg = 0; //for (Gene g : allGenes) { while (geneItr.hasNext() //&& batchg < 2 ) { Collection<Gene> pars =
         * new ArrayList<Gene>(); int count = batchSize; // work with a small batch while (geneItr.hasNext() && 0 <
         * count) { Gene g = (Gene) geneItr.next(); doesn't exist: "+ g.getId()); continue; } pars.add(g); count--; //
         * count down the number of genes in a batch } ppr.println("#Batch Number: "+ batchg); batchg++; ppr.flush();
         * outputAll_probelevel(eeCol, pars, ppr); } ppr.close(); if (true) return null;
         */
        // ///////////////////////////////////////////////// End mess!!!!!!!!!!
        ...
        while ( recordItr.hasNext() ) {
            ...
            while ( recordItr.hasNext() && 0 < count ) {
                ...
                String allEntries = ParID + "," + GeneId + "," + ParName + "," + Chrom + "," + Nuc + "," + GeneSymbol
                        + "," + Distance + "," + GeneContainsPar + "," + SameStrand;
            ...
            /*
             * pxx.println("Batch Number: "+ batch); pxe.println("Batch Number: "+ batch); pex.println("Batch Number: "+
             * batch); pee.println("Batch Number: "+ batch); ppr.println("Batch Number: "+ batch);
             * pgp_xx.println("Batch Number: "+ batch); pgp_xe.println("Batch Number: "+ batch);
             * pgp_ex.println("Batch Number: "+ batch); pgp_ee.println("Batch Number: "+ batch);
    ...
    private void printExperimentTypes( Collection<ExpressionExperiment> eeCol ) {
        ...
        for ( ExpressionExperiment e : eeCol ) {
            ...
            Collection<ArrayDesign> ad = expressionExperimentService.getArrayDesignsUsed( e );
        ...
        while ( pItr.hasNext() ) {
            ...
            for ( Gene g : coexp.keySet() ) {
                ...
                for ( CoexpressionValueObject cvo : cvos ) {
                    ...
                    pco.println( parFileEntries.get( par.getId() ) + "," + cvo.getPositiveScore() + ","
                            + cvo.getNegativeScore() );
    ...
    /*
     * Looks at all par/gene pairs to see if there are any experiemnts with both and extracts the rank information for
     * both To get the PAR-gene-experiment map, parse the output to standard out This outputs A LOT of information to
     * standard out!! Note: the stdout can be parsed by pargeneExplist.pl which sorts the par/gene/exp combo into lists.
     */
    private void outputPARGeneCorank( Collection<ExpressionExperiment> eeCol, Collection<Gene> pars,
            Collection<Gene> genes, PrintStream px, PrintStream pe, String rankMethodStr ) {
        ...
        for ( ExpressionExperiment ee : expressRankings.keySet() ) {
            ...
            while ( pItr.hasNext() ) {
                ...
                if ( !allGeneRankings.containsKey( p ) ) allGeneRankings.put( p, new ArrayList<Double>() );
                if ( !allParRankings.containsKey( p ) ) allParRankings.put( p, new ArrayList<Double>() );
        ...
        while ( gItr.hasNext() ) {
            ...
            for ( Double d : allParRankings.get( p ) ) {
                ...
                if ( maxrankPar < rank ) maxrankPar = rank;
                meanrankPar += rank;
            }
            ...
            for ( Double d : allGeneRankings.get( p ) ) {
                double rank = d.doubleValue();
                if ( maxrankGene < rank ) maxrankGene = rank;
                meanrankGene += rank;
            }
            meanrankGene = meanrankGene / numSamplesGene;

            px.println( parFileEntries.get( p.getId() ) + "," + numExpsPar + "," + numSamplesPar + "," + maxrankPar
                    + "," + numExpsGene + "," + numSamplesGene + "," + maxrankGene );
            pe.println( parFileEntries.get( p.getId() ) + "," + numExpsPar + "," + numSamplesPar + "," + meanrankPar
                    + "," + numExpsGene + "," + numSamplesGene + "," + meanrankGene );
        ...
        for ( Gene g : allProbeRankings.keySet() ) {
            ...
            for ( DesignElement de : allProbeRankingsGenes.keySet() ) {
                ...
                for ( Double[] entry : allProbeRankingsGenes.get( de ) ) {
                    ...
                    if ( valExpMax_RankMean < valRankMean ) valExpMax_RankMean = valRankMean;
                ...
                p.println( parFileEntries.get( g.getId() ) + "," + de.getId() + "," + numOfExperiments + ","
                        + valExpMean_RankMean + "," + valExpMean_RankMax + "," + valExpMax_RankMean + ","
                        + valExpMax_RankMax );
    ...
    private void outputRankings( Collection<ExpressionExperiment> eeCol, Collection<Gene> pars, PrintStream px,
        ...
        for ( Gene g : allGeneRankings.keySet() ) {
            ...
            pe.println( label + "," + numExperiments + "," + numSamples + "," + meanrank );
     ...
     * a LONG time to finish (order of 10+ days) and it might run out of memory. May need to split into other jobs or
        ...
        while ( pItr.hasNext() ) {
            ...
            for ( ExpressionExperiment ee : exps ) {
                ...
                for ( DoubleVectorValueObject dvvo : dvvos ) {
                    ...
                    if ( isParGene == false ) {
                        System.out.println( "Probe not assigned, skipping: " + dvvo.getDesignElement()
                                + " (Experiment: " + dvvo.getExpressionExperiment().getId() + ")" );
                        continue;
                    }
                ...
                for ( DesignElement pd : parData.keySet() ) {
                    ...
                    for ( DesignElement gd : geneData.keySet() ) {
                        ...
                        pap.println( parFileEntries.get( par.getId() ) + "," + ee.getId() + "," + parProbeMappings
                                + "," + geneProbeMappings + "," + pdata.length + "," + pd.getId() + "," + gd.getId()
                                + "," + corr );
