...
public class GraphViewer implements PropertyChangeListener, ActionListener, WindowListener {
    ...
    private void initVisualization( Graph g ) {
        ...
        /*
         * final GraphDistanceFilter filter = new GraphDistanceFilter("graph", hops); final JValueSlider slider = new
         * JValueSlider("Distance", 0, hops, hops); slider.addChangeListener(new ChangeListener() { public void
         * stateChanged(ChangeEvent e) { filter.setDistance(slider.getValue().intValue()); vis.run("draw"); } });
         * slider.setBackground(Color.WHITE); slider.setPreferredSize(new Dimension(300,30)); slider.setMaximumSize(new
         * Dimension(300,30)); Box cf = new Box(BoxLayout.Y_AXIS); cf.add(slider);
         * cf.setBorder(BorderFactory.createTitledBorder("Connectivity Filter")); fpanel.add(cf);
